...
from PyQt5.QtWidgets import QApplication, QFileDialog, QHeaderView, QMenu, QSystemTrayIcon, QTableWidgetItem, QWidget
...
class MainPage(QWidget, Ui_Form):
    ...
    # 加载歌曲功能
    def loadSongs(self):
        self.folder_path = QFileDialog.getExistingDirectory(self, "选择音乐文件夹")
        if self.folder_path:
            self.MusicList.load_songs_from_folder(self.folder_path)
            self.search_song(self.FilterMusic.text())
    ...
    def load_songs_to_table(self):
        ...
        for row, song in enumerate(self.MusicList.songs):
            self.tableWidget.setItem(row, 0, QTableWidgetItem(song.title))
            mod_time_str = song.ChangeTime.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def play_song(self, song, start_position=0):
        ...
        self.MusicTitle.setText(song.title)
    ...
    def display_lyrics(self):
        ...
        try:
            ...
            self.SongLyrics.setStyleSheet("QListWidget::item { padding-top: 10px; padding-bottom: 10px; max-width: 200px; }")
    ...
    def update_lyrics_display(self, current_time):
        ...
        if current_line_index != -1:
            ...
            self.SongLyrics.scrollToItem(self.SongLyrics.item(current_line_index), QtWidgets.QAbstractItemView.PositionAtCenter)
